...
################################################################################
## FOR SPEED: I prefiltered MGE-associated genes and ARGs using the bash script filter-proteins.sh.
## keep my R regexes in this code, in case there are subtle discrepancies between R regexes
## and the minimal unix regex versions generated by ChatGPT in filter-proteins.sh.
USE_PREFILTERED_GENES <- TRUE

if (USE_PREFILTERED_GENES) { ## use prefiltered data on disk.
    ground.truth.singleton.ARGs <- data.table::fread(
                                                   "../results/filtered-AMR-proteins.csv",
                                                   drop="sequence") %>%
        filter(count == 1) %>%
        inner_join(ground.truth.gbk.annotation, by="Annotation_Accession")

    ground.truth.singleton.MGEs <- data.table::fread(
